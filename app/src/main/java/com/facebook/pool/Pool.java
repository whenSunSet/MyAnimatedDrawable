...
import com.facebook.trimmable.MemoryTrimType;
 ...
 * {@link #get(int)} - 返回一个大于或者等于传入值的池子
 * {@link #get(int)} - returns a value of size that's the same or larger than the requested size
 ...
 * calls to {@link MemoryTrimmable#trim(MemoryTrimType)} (MemoryTrimType)}. Some percent (perhaps all) of the
